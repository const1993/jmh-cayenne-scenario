...
public class StubPrefetchSelect {



    @Benchmark()
    public void test_select_prefetch_one(StubState state) {
    ...
    @Benchmark()
    public void test_select_where_one(StubState state) {
        ObjectContext context = state.context;
    ...
    @Benchmark()
    public void test_select_prefetch_batch10x(StubState state) {
        ObjectContext context = state.context;
        for (int i = 0; i< 10; i++) {
            ObjectSelect.query(Artist.class).prefetch(Artist.PAINTINGS.joint()).localCache().select(context);
        }
    ...
    @Benchmark()
    public void test_select_where_batch10x(StubState state) {
        ObjectContext context = state.context;
        for (int i = 0; i< 10; i++) {
